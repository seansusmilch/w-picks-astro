...
function MatchupCell({ matchup }: { matchup: MatchupType }) {
  const awayTeamShort =
    TeamMap[matchup.away_code as keyof typeof TeamMap]?.name_short || matchup.away_code;
  const homeTeamShort =
    TeamMap[matchup.home_code as keyof typeof TeamMap]?.name_short || matchup.home_code;

  return (
    <div>
      <a href={`/matchup/${matchup.code}`} className='flex flex-col'>
      ...
      <TableBody>
        {picks.map((p) => (
          <TableRow key={p.id}>
            ...
            <TableCell>
              {p.expand?.matchup && <MatchupCell matchup={p.expand.matchup} />}
              {p.comment && <CommentCell comment={p.comment} />}
            </TableCell>
            <TableCell>
              <span
                className={`font-bold ${
                  p.result === 'W' ? 'text-primary' : 'text-destructive'
                }`}
              >
                {p.result}
              </span>
...
function NoPicks({ type }: { type: 'past' | 'live' | 'upcoming' }) {
  return (
    <p className='text-center text-muted-foreground'>No {type} picks at this time</p>
  );
}
