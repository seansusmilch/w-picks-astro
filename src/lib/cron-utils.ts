 ...
 */
export function getCronLogger(name: string): Logger & {
  trackBatchOperation: (
...
export class BatchOperationTracker {
  ...
  constructor(logger: Logger, options: BatchOperationOptions) {
    this.logger = logger;
  ...
  return fn()
    ...
    .catch((error) => {
      ...
      loggerInstance.error(
        { function: fnName, durationMs: elapsedMs.toFixed(2), error },
...
// Make sure we explicitly export everything to avoid the "not a module" error
export default {
  trackPerformance,
